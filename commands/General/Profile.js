...
module.exports = {
    ...
    run: async (interaction, client, user, language) => {
        ...
        canvas.setColor(KColors.DarkGreen)
            .printRoundedRectangle(250, 170, canvas.measureText(userPlan).width + 150, 60, radius(5))
            .setColor(KColors.White)
            .setTextFont('40px Rubik-Medium')
            .printText(userPlan, 260, 210);
        ...
        if (!top10) {
            ...
            top10.forEach((d, i=0) => {
                ...
                if (canvas.measureText(d.track_title).width > 700) {
                    ...
                    canvas.setColor(KColors.White)
                        .setTextFont('30px Rubik')
                        .printText(`${subText(d.track_title, cutLength)}`, (Number(d.track_count) > 9 ? 165 : 145), 340 + (i * 60));
                } else {
                    canvas.setColor(KColors.White)
                        .setTextFont('30px Rubik')
                        .printText(`${d.track_title}`, (Number(d.track_count) > 9 ? 165 : 145), 340 + (i * 60));
                }
            });
        };
